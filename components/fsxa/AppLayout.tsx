import { FSXABaseAppLayout, FSXAGetters, AppLocale, RequestRouteChangeParams } from "fsxa-pattern-library";
import { Page, Dropdown, Navigation, Option, NavigationItem, Footer, FooterLink } from "fsxa-ui";
...
class AppLayout extends FSXABaseAppLayout{
  ...
  renderFooter(){
    const handleLinkClick = (link: FooterLink) => {
      ...
      if (nextPage) this.handleRouteChangeRequest({
        pageId: nextPage.id,
    ...
    if(this.settings){
      ...
      const links: FooterLink[] = this.settings.data.gc_linklist
        .filter(
          link => link.data.lt_link.referenceType && link.data.lt_link.referenceId
        ).map(
          link => {
            return{
              referenceId: link.data.lt_link.referenceId,
              label: link.data.lt_text,
              isActive: link.data.lt_link.referenceType === "PageRef" && link.data.lt_link.referenceId === this.currentPage?.id,
              referenceType: link.data.lt_link.referenceType === "PageRef" ? "page" : "fragment",
            }
          }
        )
      return (
