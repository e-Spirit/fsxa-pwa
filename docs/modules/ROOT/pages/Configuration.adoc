...
This external dependency is tried to be loaded from the ContentCreator (top-frame), but a fallback is loaded from a CDN if it can't be loaded from the ContentCreator directly. This fallback could be overriden with the following options.
...
`fsTppCdnUrl` _optional_

You may want to override the used external CDN URL. If a the placeholder `{version}` exists it will be overriden by the version (see above).

*Default*: `"https://cdn.jsdelivr.net/npm/fs-tpp-api@{version}/snap.js"`
...
----
{
  ...
  fsxa: {
    ...
    logLevel: 3,
    fsTppVersion: '2.2.1',
    fsTppCdnUrl: 'https://cdn.jsdelivr.net/npm/fs-tpp-api@{version}/snap.js',
  },
  ...
}
----
